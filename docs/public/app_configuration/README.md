...
|  `VITE_APP_NAME` | Set the application name shown on the Login screen and browser title. Note this settings is overuled by the the `<title>` configured in the `WebOperoratClient.xml` in the FEWS configuration. This might change in the future. |  
...
| `VITE_LOGIN_BUTTON_PROPS` | An object or JSON string containing properties that are passed to the login button. See https://vuetifyjs.com/en/api/v-btn/#props for which property can be set. Helpful propeties are `"prependIcon"`, `"text"`, `"appendIcon"` and `"color"`. |