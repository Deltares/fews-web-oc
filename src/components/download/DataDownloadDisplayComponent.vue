    ...
    <v-container>
        <v-card title="Make a selection to download data">
            ...
            <v-row align="start" class="ma-1">
                <v-col>
                    ...
                    <template v-slot:selection="{ item, index }">
                        ...
                        <span v-if="index == 4">... ({{selectedParameters.length}} selected)</span>
                    </template>
                </v-col>
            </v-row>
            <v-row align="start" class="ma-1">
            ...
            <v-card-actions>
                <v-row class="ma-1">
                    <v-col>
                        <v-btn prepend-icon="mdi-download" color="primary" @click="downloadData" block>Download</v-btn>
                    </v-col>
                </v-row>
            </v-card-actions>
        </v-card>
        <v-row>
            <v-col>
                <v-text-field v-for="error in errors" style="color: red" readonly>{{ error }}</v-text-field>
            </v-col>
        </v-row>
    </v-container>
</template>
...
import {DocumentFormat, PiWebserviceProvider, TopologyNode} from "@deltares/fews-pi-requests"
import {DateTime} from "luxon"
import {configManager} from "@/services/application-config";
import {createTransformRequestFn} from "@/lib/requests/transformRequest.ts";
import {LocationsFilter, TimeSeriesFilter} from "@deltares/fews-pi-requests/lib/types/requestParameters";
...
import {
    TimeSeriesParameter
} from "@deltares/fews-pi-requests/lib/types/response/timeseriesparameters/timeSeriesParametersResponse";
...
const startDateString = ref<String>(DateTime.fromJSDate(getStartDateValue()).toFormat(DATE_FMT))
const endDateString = ref<String>(DateTime.fromJSDate(getEndDateValue()).toFormat(DATE_FMT))
...
watch(startDate, (newValue) => {
    let currentDisplayTime: DateTimeMaybeValid = DateTime.fromFormat(startDateString.value, DATE_FMT)
...
watch(endDate, (newValue) => {
    let currentDisplayTime: DateTimeMaybeValid = DateTime.fromFormat(endDateString.value, DATE_FMT)
    copyCurrentHoursAndMinutesToNewDateValue(currentDisplayTime, newValue);
    endDateString.value = DateTime.fromJSDate(newValue).toFormat(DATE_FMT)
})
function copyCurrentHoursAndMinutesToNewDateValue(currentDisplayTime:DateTimeMaybeValid, newValue: UnwrapRef<Date>) {
...
function getStartDateValue() {
    ...
    startDateValue.setMilliseconds(startDateValue.getMilliseconds() - 1000 * 60 * 60 * 24 * 14)
...
watch(() => selectedAttributes.value,
    async (currentValue) => {
        const attributeIds = props.topologyNode.dataDownloadDisplay?.attributes.map(item => item.id)
        selectedLocations.value = getSelectedLocations(locations.value, attributeIds, selectedAttributes.value)
    },
    {deep: true}
);
...
async function downloadData() {
    errors.value = []
    let newErrors = []
    if (selectedLocations.value === undefined || selectedLocations.value.length == 0) newErrors.push("Select one or more locations")
    if (selectedParameters.value === undefined || selectedParameters.value.length == 0) newErrors.push("Select one or more parameters")
    ...
    let queryStartDateString = startTime.toISO({suppressMilliseconds: true}) ?? ''
...
function getLocationName(location: Location): string {
    const showLocationName = props.topologyNode.dataDownloadDisplay?.showLocationName;
    if (showLocationName === 'name') return location.locationName ?? location.locationId
    if (showLocationName === 'short name') return location.shortName ?? location.locationId
    return location.locationId
}

function getParameterName(parameter: TimeSeriesParameter): string {
    const showParameterName = props.topologyNode.dataDownloadDisplay?.showParameterName;
    if (showParameterName === 'name') return parameter.name ?? parameter.id
    if (showParameterName === 'short name') return parameter.shortName ?? parameter.id
    return parameter.id
}
...
function getAttributeValues(locations: Location[]): Map<string, []> {
    ...
    const configuredAttributeIds = [...props.topologyNode.dataDownloadDisplay?.attributes.map(item => item.id)]
...
function getSelectedLocations(allLocations: Location[], attributes: String[], selectedAttributes: String[][]): Location[] {
...
function isSelected(location: Location, selectedValues: String[][], attributes: String[]) {
    for (let i = 0; i < attributes.length; i++) {
        if (selectedValues[i] === undefined || location.attributes == undefined) continue
        const foundAttribute = location.attributes?.find((attribute) => attribute.id === attributes[i])
        if (foundAttribute == undefined || foundAttribute.value == undefined) continue
