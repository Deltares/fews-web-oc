...
export const usePermissionsStore = defineStore(
  ...
  () => {
    ...
    function setPermissions(updatedPermissions: Record<string, boolean>): void {
      const newExcludedPermissionIds = new Set(excludedPermissionIds.value)
      Object.entries(updatedPermissions).forEach(
        ([currentPermissionId, isCurrentActive]) => {
          if (isCurrentActive) {
            newExcludedPermissionIds.delete(currentPermissionId)
          } else {
            newExcludedPermissionIds.add(currentPermissionId)
          }
        },
      )
