...
export class Colormap {
  ...
  toTexture(gl: WebGL2RenderingContext, numPoints: number): WebGLTexture {
      const colormapTexture = createTexture(
        gl, gl.LINEAR, this.to1DRGBTextureData(numPoints), numPoints, 1
      )
  ...
  private to1DRGBTextureData(numPoints: number): Uint8Array {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      if (i == 0) {
        ...
        color = new Color(
          this.colors[indexPrev].r * weightPrev + this.colors[indexNext].r * weightNext,
          this.colors[indexPrev].g * weightPrev + this.colors[indexNext].g * weightNext,
          this.colors[indexPrev].b * weightPrev + this.colors[indexNext].b * weightNext
        )
      }
