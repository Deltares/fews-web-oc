...
export class ShaderProgram {
  ...
  getAttributeLocation(name: string): number {
    ...
    if (location === undefined) throw new Error(`No attribute "${name}" exists.`)
    return location
  }
  ...
  static fromShaderSources(
    ...
    if (transformFeedbackVaryings) {
      gl.transformFeedbackVaryings(
        shaderProgram, transformFeedbackVaryings, gl.SEPARATE_ATTRIBS
      )
    }
...
export function bindAttribute(
  ...
  const stride = 0
  const offset = 0
  gl.vertexAttribPointer(attribute, numComponents, type, doNormalise, stride, offset)
  gl.bindBuffer(gl.ARRAY_BUFFER, null)
}
